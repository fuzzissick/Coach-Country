...
public class SessionSummary extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int sessionNum = i.getIntExtra("sessionNum", 0);
        ...
        if(sessionNum == 0){
            ...
            home.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    startActivity(new Intent(SessionSummary.this, MainActivity.class));
                    finish();
                }
            });
            setSessionSummary(sessionNum);
        }

    }

    private void setSessionSummary(int sessionNum){
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        FirebaseAuth mFirebaseAuth = FirebaseAuth.getInstance();
        FirebaseUser mFirebaseUser = mFirebaseAuth.getCurrentUser();

        db.collection("runners").document(mFirebaseUser.getUid()).collection("sessions").document("session" + sessionNum).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
